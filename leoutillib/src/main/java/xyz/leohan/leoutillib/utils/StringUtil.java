...
public class StringUtil {
    ...
    public static boolean isCorrectPhone(String phone){
        if(Pattern.compile("(^(((0)|(8))?([0-9]{2,3}))?((-)|())?([2-9][0-9]{6,7})?$)").matcher(phone).matches()||Pattern.compile("^1[3|5|7|8][0-9]\\d{8}$").matcher(phone).matches()){
            return true;
        }else{
            return false;
        }
//    if(Pattern.compile("(^(\\d{3,4}-)?\\d{7,8})$|0?(13|14|15|17|18)[0-9]{9}").matcher(phone).matches()){
//            return true;
//        }else{
//            return false;
//        }
     ...
     * @return boolean
     */
    public static boolean isCorrectIdentity(String identity) {
        if(new IDCard().verify(identity)){
            return true;
        }else{
            return false;
        }
